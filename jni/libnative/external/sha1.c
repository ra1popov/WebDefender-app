...
// initialises given SHA-1 context
...
//		temp = rotl32(a[0],5) + (dtemp=f[fi](a[1],a[2],a[3])) + a[4] + k[fi] + w[s];
		switch (fi)
		{
		case 0: dtemp = ch(a[1], a[2], a[3]); break;
		case 1: dtemp = parity(a[1], a[2], a[3]); break;
		case 2: dtemp = maj(a[1], a[2], a[3]); break;
		case 3: dtemp = parity(a[1], a[2], a[3]); break;
		default: dtemp = 0;
		}
		temp = rotl32(a[0], 5) + dtemp + a[4] + k[fi] + w[s];
...
void sha1_nextBlocks (sha1_ctx_t *state, const void* blocks, uint16_t count)
{
	while (count > 0)
	{
		sha1_nextBlock(state, blocks);
		count--;
		blocks = (uint8_t*) blocks + SHA1_BLOCK_BYTES;
	}
}
...
{
	while (length >= SHA1_BLOCK_BYTES)
	{
		sha1_nextBlock(state, blocks);
		length -= SHA1_BLOCK_BYTES;
		blocks = (uint8_t*) blocks + SHA1_BLOCK_BYTES;
	}
	sha1_lastBlock(state, blocks, length);
}
...
void sha1 (void *dest, const void* msg, uint32_t length)
{
	...
	while (length >= SHA1_BLOCK_BYTES)
	{
		...
		//length -= SHA1_BLOCK_BITS;
		msg = (uint8_t*) msg + SHA1_BLOCK_BYTES; // increment pointer to next block
		length -= SHA1_BLOCK_BYTES;
	}
